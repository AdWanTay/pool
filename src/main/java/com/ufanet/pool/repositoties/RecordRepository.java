...
import com.ufanet.pool.models.dto.ReservationByNameRequest;
...
public interface RecordRepository extends JpaRepository<Record, Long> {
    ...
    @Query("SELECT new com.ufanet.pool.models.dto.ReservationByNameRequest(c.id, c.name, tt.date,tt.time) " +
            "FROM Record tt left join Client c on tt.client.id = c.id " +
            "where c.name = :name")
    Optional<List<ReservationByNameRequest>> findAllByClientName(@Param("name") String name);
